...
export default function PesquisarTabela<T> ( {
  ...
  const pesquisaValorInicialCampoPorId = () => {
    ...
    if ( ( typeof dados[fieldSet] === 'number' && dados[fieldSet] > 0 )
      || ( typeof dados[fieldSet] === 'string' && dados[fieldSet].length > 0 )
    ) {

      if ( rsPesquisa.length > 0 ) {
        indiceRegistro = rsPesquisa.findIndex( v => v[campoQueryPesquisaID as any] === dados[fieldSet] )
      }
      ...
      if ( indiceRegistro < 0 ) {
        ...
        if ( typeof dados[fieldSet] === 'string' ) {
          query = `${nomeQueryPesquisaID}(${campoQueryPesquisaID}: "${dados[fieldSet]}") ${camposRetornoQueryPesquisa}`
        } else {
          query = `${nomeQueryPesquisaID}(${campoQueryPesquisaID}: ${dados[fieldSet]}) ${camposRetornoQueryPesquisa}`
        }
